...
//  def store(spGUIState: SPGUIModel) = println("BrowserStorage store" + spGUIState)
//  def load(): Unit = println("BrowserStorage load")
//}
//
package spgui.circuit
...
object BrowserStorage {
  val namespace = "SPGUIState"
  def store(spGUIState: SPGUIModel) = println(spGUIState.toString)
  def load: Option[SPGUIModel] = namespace match {
      case _ => None
    }
}